...
dir = 'Z:/Learning/Advent of Code/2024/05/b'
os.chdir(dir)
...
data_ex = data_ex[data_ex['output'] == False]
        ...
        if sum(rules['number'] == numbers[i]) == 0:
            ...
            for j in range(len(check)):
                if check[j] in numbers:
                    index = numbers.index(check[j])
                    if i > index:
                        numbers.insert(i, numbers.pop(numbers.index(check[j])))
                        i -= 1
                        j = len(check)
                    else:
                        pass
...
data_ex['reordered'] = data_ex['input'].apply(reorder)
...
data_ex['middle'] = data_ex['reordered'].apply(find_middle)
...
def main(data):
    ...
    data['reordered'] = data['input'].apply(reorder)
    ...
    data['middle'] = data['reordered'].apply(find_middle)
